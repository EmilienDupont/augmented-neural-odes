...
from experiments_2.torch_ode.common import torch_select_initial_step, torch_rms_norm
...
class TorchRK45(TorchODESolver):
    ...
    def __init__(self, step_size: [float, str] = 0.01, rtol=1e-3, atol=1e-6):
    ...
    def solve_ivp(self, func: Callable[[float, torch.Tensor, ...], torch.Tensor], t_span: Tuple,
        ...
        while not finished:
            ...
            z, f, h, t = TorchRK45._torch_rk_step_adaptive_step(func=func, t=t, tf=tf, z=z, f=f, h=h, A=TorchRK45.A,
                                                                B=TorchRK45.B, C=TorchRK45.C, K=self.K, atol=self.atol,
                                                                rtol=self.rtol)
    ...
    def _torch_rk_step_adaptive_step(func: Callable, t: float, tf: float, z: torch.Tensor, f: torch.Tensor,
                                     h: float,
                                     A: torch.Tensor, B: torch.Tensor, C: torch.Tensor, K: torch.Tensor, rtol: float,
        ...
        while not step_accepted:
            ...
            error_norm = TorchRK45._estimate_error_norm(K, h, scale)
    ...
    @staticmethod
    def _estimate_error(K, h):
        return torch.matmul(K.T, TorchRK45.E) * h

    @staticmethod
    def _estimate_error_norm(K, h, scale):
        return torch_rms_norm(TorchRK45._estimate_error(K, h) / scale)
